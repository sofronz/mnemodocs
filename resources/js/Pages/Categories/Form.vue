...
const form = useForm({
  ...
  status: props.isEdit ? ref(props.category.data.status) : ref(true),
})

// Submit handler
const submit = () => {
  if (props.isEdit) {
    form.put(route('categories.update', props.category.data.id)) // Update jika mode edit
  } else {
    form.post(route('categories.store')) // Create jika mode baru
  }
}
</script>
...
<template>
  <LayoutAuthenticated>
    ...
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiPalette" :title="isEdit ? 'Edit Category' : 'Create Category'" :show-button="false"
        main />
